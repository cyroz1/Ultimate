...
if (Stop-UltimateArm64UnsupportedFeature -Feature 'The AMD graphics settings preset' -Reason 'It writes AMD-only registry settings across display-adapter keys. ARM64 device graphics settings should be managed by the device manufacturer.') {
    Pause
    exit
}
